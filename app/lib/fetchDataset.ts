...
export async function fetchDataset(): Promise<NotionDataset[]> {
  const pages = await queryNotionDatabase(
    NOTION_DB.jeuxDeDonnees,
    APPROVED_FILTER,
  ...
  return pages.map((page) => {
    ...
    return {
      ...
      catalog_parent_ids: getRelationIds(
        props,
        "Catalogues de données parent",
      ),
